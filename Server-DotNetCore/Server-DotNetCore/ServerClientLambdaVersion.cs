...
public class ServerClientLambdaVersion
{
    private readonly TcpClient _client;
    private readonly NetworkStream _stream;
    private readonly Dictionary<Command, Func<NetworkStream, Task>> _handlers;

    public ServerClientLambdaVersion(TcpClient client)
    {
        _client = client;
		...
		_handlers = new Dictionary<Command, Func<NetworkStream, Task>>
        {
            ...
            {
                Command.ListFiles, async (stream) =>
                {
                    string[] files = Directory.GetFiles("server_files");
                    await stream.WriteAsync(BitConverter.GetBytes(files.Length));
